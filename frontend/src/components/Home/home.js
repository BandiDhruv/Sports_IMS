...
function Home() {
  ...
  async function getData(){
    try{  
      const res=await axios.get("https://sports-ims.onrender.com/InventoryDetails",{withCredentials: true});
      console.log(res);
      setData(res.data);
    }catch(e){
      console.log(e);
  ...
  const buttonClick = (text) => {
    ...
    let cards = data ? data.map((item) => ({
      key: uuidv4(),
      content: (
        <Card
          imagen={item.sportImage}
          text={item.sportName}
          paragraph={item.description}
          onClick={() => buttonClick(item.sportName)}
        />
      )
    })) : [];
    ...
    <div className="home-container">
      ...
      <div className="home-div">
        <div className="left">
          ...
          {data && data.map((item)=>{
            return(
              <li key={item.sportName}  className="left-panel-list" >
                <Link className="left-panel-link" to={`/Sports/${item.sportName}`} >{item.sportName}</Link>
              </li>
            )
          })}
          </div>
