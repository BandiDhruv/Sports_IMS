...
const ManageItems = () => {
  ...
  async function increment(sport,id) {
    try {
        console.log(id);
      const res = await axios.patch(`${process.env.BACKEND_IP}/increment-quantity/${sport}/${id}`, {withCredentials: true});
  ...
  async function decrement(sport,id) {
    try {
        console.log(id);
      const res = await axios.patch(`${process.env.BACKEND_IP}/decrement-quantity/${sport}/${id}`, {withCredentials: true});
      console.log(res.data); // Check the response
      setTemp(temp-1);
    } catch (err) {
      console.error("Error incrementing quantity", err);
